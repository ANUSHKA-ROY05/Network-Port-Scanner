...
# ---------------Scan Type Dropdown -----------------------------------------
        ttk.Label(frm_top, text="Scan Type:").grid(row=1, column=0, padx=8, pady=8, sticky="e")

        self.scan_type = ttk.Combobox(frm_top, values=["Quick", "Normal", "Full"], state="readonly")
        self.scan_type.set("Normal")
        self.scan_type.grid(row=1, column=1, padx=8, pady=8, sticky="w")
    ...
    def start_scan(self):
        ...
        if not target:
            ...
            elif scan_choice == "Normal":
                start_port, end_port = 1, 1024
            elif scan_choice == "Full":
                start_port, end_port = 1, 65535
        else:
            try:
    # -------------------fallback to manual input (optional)----------
